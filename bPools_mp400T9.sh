...
install400T9(){
    ...
    if [ -f "/root/miner_Bpools/patch_MP400T9" ]; then
      rm -rf /root/miner_Bpools/patch_MP400T9
    fi
      
    if [ -f "/root/miner_Bpools/minerProxy400T9" ]; then
      rm -rf /root/miner_Bpools/minerProxy400T9
    fi
    if [ -f "/root/miner_Bpools/licMaker" ]; then
      rm -rf /root/miner_Bpools/licMaker
    fi
    
    mkdir -p /root/miner_Bpools
    mkdir -p /root/miner_BpoolsTemp
    ...
    # tar -zxvf /root/Bpools_AntiMP_Linux.tar.gz -C /root/miner_BpoolsTemp
    
    #清空下载目录
    rm -rf /root/miner_BpoolsTemp    
    
    #开始下载代码
    git clone https://github.com/minerproxys/Bpools -b main /root/miner_BpoolsTemp --depth=1

    if [[ ! -d /root/miner_BpoolsTemp ]]; then
        echo
    ...
    /bin/cp -rf /root/miner_BpoolsTemp/Bpools_400T9_linux/Bpools ${installPath}/
    /bin/cp -rf /root/miner_BpoolsTemp/Bpools_400T9_linux/patch_MP400T9 ${installPath}/
    /bin/cp -rf /root/miner_BpoolsTemp/Bpools_400T9_linux/minerProxy400T9 ${installPath}/
    /bin/cp -rf /root/miner_BpoolsTemp/Bpools_400T9_linux/licMaker ${installPath}/
    /bin/cp -rf /root/miner_BpoolsTemp/Bpools_400T9_linux/upx ${installPath}/
    ...
    chmod 777 /root/miner_Bpools/patch_MP400T9
    chmod 777 /root/miner_Bpools/minerProxy400T9
    chmod 777 /root/miner_Bpools/licMaker
    chmod 777 /root/miner_Bpools/upx
    
    
    #处理授权证书
    if [ -f "/root/miner_Bpools/授权证书.lic" ]; then
      echo "授权证书.lic 已保留"
    else
    #如果证书不存在
    #手动制作：
    ...
    #制作失败的话，使用作者证书
        if [ -f "/root/miner_Bpools/授权证书.lic" ]; then
      		...
      		/bin/cp -rf /root/miner_BpoolsTemp/Bpools_400T9_linux/授权证书.lic ${installPath}
      	fi
    fi
    ...
    echo "开始配置破解，如果半路中断，请重新安装"
    
    #停止Bpools，避免wallet.yml被其改写
    supervisorctl stop    Bpools
    
    #开始设置 重定向端口，web端口，并破解程序
    ./patch_MP400T9
    ...
    echo "1.请记录以下端口、密码等数据。然后浏览器打开 ip:web端口 ，登录Bpools网页查看反抽"
    echo "----------------------------------------------------------------"
    echo "Bpools参数："
...
temp(){
    echo  "相关破解，请在 https://github.com/minerproxys 上下载"
    echo  "或者联系 https://t.me/MinerProxyHackGO"
}
